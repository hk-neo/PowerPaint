...
if __name__ == "__main__":
    ...
    args.add_argument("--input", type=str, default="inputPath")
    args.add_argument("--mask", type=str, default="maskPath")
    args.add_argument("--controlnet", type=str, default="controlnetPath")
    args.add_argument("--control_type", type=str, default="canny")
    args.add_argument("--controlnet_conditioning_scale", type=float, default=0.5)
    
    args.add_argument("--output", type=str, default="outputPath")
    args.add_argument("--text_prompt", type=str, default="textPrompt")
    args.add_argument("--text_negative_prompt", type=str, default="textNegativePrompt")
    ...
    print(args.text_negative_prompt)
    print(args.version)
    print(args.share)
    print(args.local_files_only)
    print(args.port)  

    # initialize the pipeline controller
    weight_dtype = torch.float16 if args.weight_dtype == "float16" else torch.float32
    script_dir = os.path.dirname(os.path.abspath(__file__))    
    checkpoint_dir = os.path.join(script_dir, "checkpoints", "ppt-v1")
    controller = PowerPaintController(weight_dtype, checkpoint_dir, args.local_files_only, args.version)
    
    image = Image.open(args.input)
    mask = Image.open(args.mask)
    input_image = {"image": image, "mask": mask}    
    text_guided_prompt = args.text_prompt
    text_guided_negative_prompt = args.text_negative_prompt
    fitting_degree = 0.5
    ddim_steps = 45
    scale = 5.5
    ...
    if args.controlnet == "controlnetPath":
        ...
        result, maks_result = controller.predict(
            input_image, 
            text_guided_prompt,         
            fitting_degree, 
            ddim_steps, 
        ...
        output_path = args.output #'D:/output_image.png' 
        result[0].save(output_path)
        
        print(f'Image saved to {output_path}')
        
    else:
        print("Start predict_controlnet")
        input_control_image = Image.open(args.controlnet)
        control_type = args.control_type
        controlnet_conditioning_scale = args.controlnet_conditioning_scale
        result, maks_result = controller.predict_controlnet(
            input_image,
            input_control_image,
            control_type,
            text_guided_prompt,
        ...
        print("Finish predict_controlnet")
